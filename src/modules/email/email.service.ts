...
export class EmailService {
  ...
  async sendLeadNotification(lead: LeadForEmail): Promise<void> {
    ...
    try {
      ...
      console.error('❌ Notification email failed:', errorMessage);
    }
  }
  ...
  async sendLeadConfirmation(lead: LeadForEmail): Promise<void> {
    try {
      await this.send({
        ...
        to:      lead.email,
        subject: `We received your inquiry, ${lead.contactName.split(' ')[0]}!`,
  ...
  // ─── Admin sends a custom email to a lead ─────────────────────────────────
  async sendCustomEmail(lead: LeadForEmail, subject: string, body: string): Promise<void> {
    const htmlBody = body
      .split('\n')
      .map((line) => `<p style="margin:0 0 12px;color:#374151;line-height:1.6">${esc(line) || '&nbsp;'}</p>`)
      .join('');
...
<style>
  ...
  .plan{color:#0369a1;font-weight:700}
...
<div class="card">
  ...
  <table>
    ...
    <tr><td>Interested Plan</td><td class="plan">${plan}</td></tr>
  ...
  <p class="footer">Lead ID: ${lead.id} · IP: ${lead.ipAddress ?? 'unknown'}</p>
</div></body></html>`;
}
...
<div class="card">
  ...
  <div class="highlight">
    ${plan ? `<p><strong>Your selected plan:</strong> ${plan}</p>` : ''}
